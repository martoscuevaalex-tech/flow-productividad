...
export function useFlow(){
  ...
  const mutate=useCallback((id:string,p:Patch)=>{
    ...
    try{const now=readBox(scope.current);commit(user?enqueue(now,id,p):applyLocal(now,id,p));if(!paused.current)setError('');}
  ...
  return {tasks:project(box),user,authReady,error,conflict,online,syncing,lastSync,lastDay:day,pending:box.queue.length,mutate,sync,resolve,importGuest,stopDaily,setError};
}
