-- Persist manual task ordering across every signed-in device.
begin;
alter table public.flow_tasks add column if not exists sort_order bigint not null default 0 check (sort_order>=0);
with ranked as (
  select id,row_number() over(partition by user_id,date order by done,case priority when 'alta' then 1 when 'media' then 2 else 3 end,id)*1024 as position
  from public.flow_tasks
)
update public.flow_tasks t set sort_order=r.position from ranked r where t.id=r.id and t.sort_order=0;
...
declare
  owner_id uuid := auth.uid(); current_row public.flow_tasks; previous public.flow_operations; result_row jsonb;
  routine public.flow_tasks; source_row public.flow_tasks; series uuid; recurring text; stop_day date; archived boolean := false;
begin
  ...
  series := (p_patch->>'series_id')::uuid; recurring := coalesce(p_patch->>'recurrence','none'); stop_day := (p_patch->>'repeat_until')::date;
  ...
  if not found then
    ...
    if p_expected_version=0 and series is not null and series<>p_task_id and current_row.series_id=series and current_row.date=(p_patch->>'date')::date then
      result_row := to_jsonb(current_row); insert into public.flow_operations(id,user_id,task_id,result) values(p_operation_id,owner_id,p_task_id,result_row); return result_row;
    end if;
    ...
    update public.flow_tasks set
      title=coalesce(p_patch->>'title',title),date=coalesce((p_patch->>'date')::date,date),category=coalesce(p_patch->>'category',category),priority=coalesce(p_patch->>'priority',priority),
      notes=coalesce(p_patch->>'notes',notes),done=coalesce((p_patch->>'done')::boolean,done),deleted=coalesce((p_patch->>'deleted')::boolean,deleted),
      ...
      repeat_until=case when p_patch ? 'repeat_until' then stop_day else repeat_until end,sort_order=coalesce((p_patch->>'sort_order')::bigint,sort_order),version=version+1,updated_at=now()
